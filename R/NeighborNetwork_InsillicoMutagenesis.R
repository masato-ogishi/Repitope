...
#' \code{InsillicoMutagenesis_PairwiseNeighborNetwork} generates single-mutated peptide sequences and construct neighbor networks.
#'
...
InsillicoMutagenesis_PairwiseNeighborNetwork <- function(peptidePairDF, coreN=NULL){
  ...
  neighborNetList <- pbapply::pblapply(
    ...
    function(i){net_singlepair(peptidePairDF[i,1], peptidePairDF[i,2])},
    cl=cl
  )
  if(!is.null(coreN)) parallel::stopCluster(cl=cl)
  gc();gc()
  return(neighborNetList)
}

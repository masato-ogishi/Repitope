  ...
  contact_potential_profiling <- function(peptideSet, aaIndexID, fragLen, fragDepth, seed, cl=cl){
    ...
    dt <- snow::parLapply(
      ...
      function(pept){
        ...
        df <- data.table::transpose(
          as.data.frame(sapply(FragLib_Tmp, function(d){calculate_stats_single(pept, d, AACPMatrix_Tmp, seed)}))
        )
    ...
    colnames(dt) <- c("Mean","SD","Med","TrM","MAD","Skew","Kurt","SE","IQR","Q10","Q90","Library")
